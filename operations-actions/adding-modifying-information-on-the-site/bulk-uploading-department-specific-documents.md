...
Visit the [Bulk Upload interface](https://datamanager.vulekamali.gov.za/admin/bulk\_upload) directly or from the menu on the [Data Manager Admin homepage](https://datamanager.vulekamali.gov.za/admin).
...
{% hint style="info" %}
**Limit the number of rows so that the upload does not time out**

If you encounter what looks like network errors, the connection was probably closed because the checks of your metadata file took too long to complete.

Try a smaller number of rows. From experience, 20-40 rows at a time has worked well.

Both the preview and submit steps do many checks of your metadata file before queueing the task to import the department files. These checks can cause the metadata file submission to time out and result in a "network error" message in your browser.
{% endhint %}
...
{% file src="../../.gitbook/assets/adjusted-budget-vote-docs-bulk-upload-example.xlsx" %}
Example metadata file for AENE 2017 vote documents
{% endfile %}
